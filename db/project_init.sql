-- ============================================================
-- 项目初始化 SQL（API 生成器表）
-- 使用前请先执行：jimureport-example/db/jimureport.mysql5.7.create.sql
-- 以创建 jimureport 库及报表相关表与基础数据。
-- 本脚本在已有 jimureport 库上创建 API 生成器所需表。
-- ============================================================
...
CREATE TABLE `api_field` (
  ...
  `is_primary` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否主键（1：是，0：否）',
...
CREATE TABLE `api_param` (
  ...
  `param_type` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL DEFAULT 'query' COMMENT '参数类型：query/path/body',
